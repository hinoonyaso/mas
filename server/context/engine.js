...
export default class ContextEngine {
     ...
     */
    buildContext(agentName, { userInput, previousSteps = [], runId }) {
        const context = {
            previousSteps: this._selectRelevantSteps(agentName, previousSteps),
    ...
    /**
     * 에이전트별로 관련 있는 이전 단계만 선택
     */
    _selectRelevantSteps(agentName, previousSteps) {
        const limits = {
